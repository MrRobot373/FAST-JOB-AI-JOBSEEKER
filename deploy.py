...
async def generate_questions_with_answers_async(resume: dict, jd: Optional[dict] = None, request_id: str = None) -> list:
    """Async Q&A generation with parallel processing - returns array format"""
    jd_section = f"\nJob Description:\n{json.dumps(jd, indent=2)}" if jd else ""
    source_instruction = (
        "Blend Resume and JD to generate realistic and relevant questions."
        if jd else
...
✅ Format: Return a valid JSON array of objects, each containing "question" and "answer" fields.

Example format:
[
  {{
    "question": "What is your experience with React?",
    "answer": "I have 3 years of experience working with React..."
  }},
  {{
    "question": "How do you handle state management?",
    "answer": "I typically use Redux for complex applications..."
  }}
]

- Generate exactly 20 question-answer pairs
- Ensure answers are professional and detailed
- Infer answers professionally even if information is limited
{source_instruction}
...
"""
    ...
    try:
        response = await gemini_manager.generate_content_async(prompt, timeout=50, request_id=request_id)
        # Parse the JSON response
        qa_array = json.loads(response.strip())
        return qa_array
    except json.JSONDecodeError as e:
        logger.error(f"❌ [{request_id}] Failed to parse Q&A JSON: {e}")
        # Fallback: return empty array or try to extract Q&A from string format
        return []
    except Exception as e:
        logger.error(f"❌ [{request_id}] Q&A generation error: {e}")
        raise e
...
async def generate_qa_api(
    ...
    try:
        ...
        return {
            "qa": result,  # This will now be an array instead of string
            "request_id": request_id,
            "user_id": user_id,
            "timestamp": datetime.utcnow().isoformat(),
            "status": "success",
